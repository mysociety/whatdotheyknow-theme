...
if ['info_requests', 'censor_rules'].all? { |table_name|
    ...
    rules_data = [{:text => '\*\*\*+\s+Polly Tucker.*',
                      :replacement => '',
                      :regexp => true,
                      :censorable => InfoRequest.find_by_url_title('total_number_of_objects_in_the_n_6'),
                  ...
                  {:text => 'Andy 079.*',
                      ...
                      :regexp => true,
                      :censorable => InfoRequest.find_by_url_title('cctv_data_retention_and_use'),
                      :last_edit_editor => 'system',
                      :last_edit_comment => 'Refactored from remove_privacy_sensitive_things!'},
                  ...
                  {:text => '(Complaints and Corporate Affairs Officer)\s+Westminster Primary Care Trust.+',
                      :replacement => '\\1',
                      :regexp => true,
                      :censorable => InfoRequest.find_by_url_title('how_do_the_pct_deal_with_retirin_113'),
                      :last_edit_editor => 'system',
                      :last_edit_comment => 'Refactored from remove_privacy_sensitive_things!'},

                  {:text => 'Your password:-\s+[^\s]+',
                      :replacement => '[password]',
                      :censorable => PublicBody.find_by_url_name('home_office'),
                      :regexp => true,
                      :last_edit_editor => 'system',
                      :last_edit_comment => 'Refactored from remove_privacy_sensitive_things!'},
                  {:text => 'Password=[^\s]+',
